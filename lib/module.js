...
var _module = function _module(_ref) {
    ...
    var checkStatus = function checkStatus(nome) {
        return _refreshWatchList().then(function (it) {
            return _.filter(it, function (v) {
                return v.nome === nome;
            });
        });
    };
    ...
    var trainStatus = function trainStatus(v) {
        ...
        return superagent.get(andamentoTreno(sp, tn)).then(function (v) {
            ...
            data = _.pick(data, ["orarioPartenza", "orarioArrivo", "stazioneUltimoRilevamento", 'destinazione', 'origine', 'dovrebbeArrivareAlle']);
